...
class Answers extends Model {
    ...
    public static function editAllAnswersIsCheckStatus($data){
        ...
        if(is_array($reply_id) && count($reply_id) > 0){
            //0禁用 1启用
            $answers_lsit = AnswersReply::whereIn('id', $reply_id)->select('id','status')->get()->toArray();
            foreach ($answers_lsit as $ak => $av){
                if($av['status'] == 1){
                    $answers_lsit[$ak]['edit_status'] = 0;
                }elseif($av['status'] == 0){
                    $answers_lsit[$ak]['edit_status'] = 1;
                }
            }
            foreach ($answers_lsit as $k => $v){
