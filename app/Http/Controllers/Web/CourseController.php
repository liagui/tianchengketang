...
//                ->leftJoin('ld_lecturer_educationa','ld_lecturer_educationa.id','=','ld_course_teacher.teacher_id')
                ->where(function ($query) use ($parent,$name) {
                    ...
                    if(!empty($name)){
                        $query->leftJoin('ld_course_teacher','ld_course_teacher.course_id','=','ld_course.id');
...
//                }
    ...
    public function courseDetail(){
        ...
        if($this->data['nature'] == 1){
            ...
            //学习人数   基数+订单数
            $ordernum = Order::where(['class_id'=>$course['class_id'],'nature'=>1])->whereIn('status',[1,2])->whereIn('pay_status',[3,4])->count();
            ...
            //学习人数   基数+订单数
            $ordernum = Order::where(['class_id'=>$course['id'],'nature'=>0])->whereIn('status',[1,2])->whereIn('pay_status',[3,4])->count();
            $course['buy_num'] = $course['buy_num'] + $ordernum;
            //讲师信息
            $teacher = [];
