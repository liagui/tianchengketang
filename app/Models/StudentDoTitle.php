...
class StudentDoTitle extends Model {
        ...
        /*$list = self::leftJoin('ld_question_exam','ld_question_exam.id','=','ld_student_do_title.exam_id')
            ...
            ->where(function($query) use ($data){
                ...
                if(!empty($data['type']) && $data['type'] > 0){
                    $query->where('ld_student_do_title.quert_type' , '=' , $data['type']);
                }
            })
            ...
            ->get()->toArray();*/
        $list = self::where(['student_id'=>$data['student_id'],'bank_id'=>$data['bank_id'],'subject_id'=>$data['subject_id']])
            ->where(function($query) use ($data){
                //判断题库id是否为空
                if(!empty($data['papers_id']) && $data['papers_id'] > 0){
                    $query->where('papers_id' , '=' , $data['papers_id']);
                }
            })
            ->select('ld_student_do_title.is_right','ld_student_do_title.answer as student_answer')
            ->get()->toArray();
        return $list;
    }
    ...
    private static function getExamInfo($data){
        return self::leftJoin('ld_question_exam','ld_question_exam.id','=','ld_student_do_title.exam_id')
            ->leftJoin('ld_question_papers','ld_question_papers.id','=','ld_student_do_title.papers_id')
            ->leftJoin('ld_question_exam_option','ld_question_exam_option.exam_id','=','ld_question_exam.id')
            ->where(['ld_student_do_title.student_id'=>$data['student_id'],'ld_student_do_title.bank_id'=>$data['bank_id'],'ld_student_do_title.subject_id'=>$data['subject_id']])
            ->where(function($query) use ($data){
                //判断题库id是否为空
                if(!empty($data['papers_id']) && $data['papers_id'] > 0){
                    $query->where('ld_student_do_title.papers_id' , '=' , $data['papers_id']);
                }
            })
