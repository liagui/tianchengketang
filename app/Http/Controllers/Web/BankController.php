...
//        $bank_list12 = Order::where(['student_id'=>self::$accept_data['user_info']['user_id'],'status'=>2,'nature'=>1])->whereIn('pay_status',[3,4])->whereIn('class_id',$courseids)->count();
    ...
    public function getExamSet(){
        ...
        //未做试题数量   总题数 - 已做题数
        $yes_exam_count = StudentDoTitle::select(DB::raw("any_value(exam_id) as exam_id"))->where("student_id" , self::$accept_data['user_info']['user_id'])->where('bank_id' , $bank_id)->where('subject_id' , $subject_id)->where('chapter_id' , $chapter_id)->where('joint_id' , $joint_id)->where('type' , 1)->where('is_right' ,'!=',0)->groupBy('exam_id')->get()->count();
