...
//                    }
                    ...
                    foreach($list  as $k=>&$v){
                        ...
                        $list[$k]['sum_nember'] = CourseStocks::where(['school_pid'=>$data['school_id'],'school_id'=>$data['school_id'],'course_id'=>$v['course_id'],'is_del'=>0])->sum('add_number');
                        $list[$k]['surplus'] = $list[$k]['sum_nember']-$v['buy_nember'] <=0 ?0:$list[$k]['sum_nember']-$v['buy_nember']; //剩余库存量
                    }
            }else{
                //自增
